...
// ブログ(お知らせ)は Content スコープが必要。専用トークンがあればそれを使い、
// 無ければ通常トークンにフォールバック(1トークンにProducts+Contentを集約した場合)。
const CONTENT_TOKEN = process.env.BIGCOMMERCE_CONTENT_TOKEN || ACCESS_TOKEN;
const clientV2Content = axios.create({
  baseURL: `https://api.bigcommerce.com/stores/${STORE_HASH}/v2`,
  headers: {
    'X-Auth-Token': CONTENT_TOKEN,
    Accept: 'application/json',
    'Content-Type': 'application/json',
  },
  timeout: 10000,
...
/**
 * List visible categories (for storefront navigation).
 * @returns {Promise<Array<{id:number, name:string, parentId:number, description:string}>>}
 */
 ...
 *
 * @param {{limit?:number, page?:number, categoryId?:number}} [opts]
 * @returns {Promise<{products:Array, pagination:object}>}
 */
async function listProducts({ limit = 50, page = 1, categoryId, keyword } = {}) {
  const params = { include: 'custom_fields,images', limit, page, is_visible: true };
  ...
  const products = data.data.map((p) => {
    ...
    const primary = imgs.find((img) => img.is_thumbnail) || imgs[0];
    // ホバー用の2枚目(中身写真など)。primary と別の画像があれば採用。
    const hover = imgs.find((img) => img !== primary);
    return {
      id: p.id,
      name: p.name,
      sku: p.sku,
...
/**
 * Fetch inventory tracking/level for the given product ids.
 * tracking 'none' means unlimited stock.
 * @param {number[]} ids
 * @returns {Promise<Object<number,{tracking:string, level:number}>>}
 */
...
/**
 * Fetch published blog posts (お知らせ). Newest first.
 * Body は管理画面のリッチエディタで作成したHTML(文字サイズ等の書式を含む)。
 * ※ トークンに Content(Read-only) スコープが必要。
 * @param {number} [limit]
 */
async function getBlogPosts(limit = 10) {
  const { data } = await clientV2Content.get('/blog/posts', {
  ...
  return (data || []).map((p) => ({
    ...
    body: p.body || '',
    summary: p.summary || '',
    publishedAt: p.published_date_iso8601 || (p.published_date && p.published_date.date) || null,
    url: p.url,
  }));
}
